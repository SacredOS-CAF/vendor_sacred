...
func init() {
	android.RegisterModuleType("sacred_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = sacredExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = sacredExpandVariables(ctx, dep_file)
	...
	cmd := sacredExpandVariables(ctx, String(g.properties.Cmd))
